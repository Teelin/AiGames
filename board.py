...
class board:
    ...
    def setMboard(self, mbstr):
        ...
        self.mbList = mbstr.split(",")
        self.validM = []
        for i in range(0, len(self.mbList)):
            if self.mbList[i] == "-1" or self.mbList[i] == "-1\n":
    ...
    def getmove(self,pid):
        ...
        if self.VMB == '0':
            ...
            if len(valid) == 9:
                ...
                if self.wincheck(self.myid,self.board6):
                    self.x += 6
                    self.y += 3
